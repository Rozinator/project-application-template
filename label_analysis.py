...
# # Loading JSON data
# with open('data/poetry_issues_all.json', 'r', encoding='utf-8') as f:
#     issues = json.load(f)
...
# #  Average resolution time computation (only closed issues) 
...
#     created = parse_date(issue.get('created_date'))
#     updated = parse_date(issue.get('updated_date'))
#     if not created or not updated:
#         continue

#     resolution_days = (updated - created).total_seconds() / (24 * 3600)
#     resolution_months = resolution_days / 30.44  # convert days to months

#     for label in issue.get('labels', []):
#         label_resolution_sum[label] += resolution_months
#         label_resolution_count[label] += 1

# avg_resolution_time = {label: label_resolution_sum[label] / label_resolution_count[label]
#                        for label in label_resolution_sum}

# # Filter the average times to the top 15 labels for aligned plotting
